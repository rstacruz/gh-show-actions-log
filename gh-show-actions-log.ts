...
class GhCli {
  ...
  static getRepo() {
    return Util.execCommand("gh repo view --json nameWithOwner -q '.nameWithOwner'", { silent: true, ignoreError: true });
  }

  static getWorkflowId(workflowName: string) {
    return Util.execCommand(`gh workflow list --json name,id --jq '.[] | select(.name | contains("${workflowName}")) | .id'`, { silent: true });
  }

  static getRunStatus(repo: string, runId: number) {
    return Util.execCommand(`gh run view --repo "${repo}" "${runId}" --json status,conclusion --jq '.'`, { silent: true });
  }
  ...
  static listRuns(repo: string, limit: number, filter: string) {
    ...
    return Util.execCommand(`gh run list --repo "${repo}" --limit "${limit}" --json databaseId,headBranch,workflowName,createdAt,status,conclusion --jq '${jqFilter}'`, { silent: true });
  }

  static getFailedJobs(repo: string, runId: number) {
    return Util.execCommand(`gh run view --repo "${repo}" "${runId}" --json jobs --jq '.jobs[] | select(.conclusion == "failure") | {name: .name, databaseId: .databaseId}'`, { silent: true });
  }

  static getJobLogs(repo: string, jobId: number) {
    return Util.execCommand(`gh run view --repo "${repo}" --job "${jobId}" --log`, { silent: true });
  }
...
class Util {
  ...
  static execCommand(command: string, options: {
    silent?: boolean;
    ignoreError?: boolean;
...
class ShowLogAction {
  ...
  static async waitForWorkflowCompletion(repo: string, runId: number): Promise<any | null> {
    let elapsed = 0;
  ...
  static async main(): Promise<void> {
    ...
    // Get failed runs
    let failedRuns = runningFailures.length > 0 ? runningFailures : GhCli.getFailedRuns(repo, LIMIT);
